...
const OrdersList = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        const response = await axios.get(
          `https://quickmeds.sndktech.online/orders.By.userId/${userId}`,
          {
            headers: {
              "x-authorization":
        ...
        if (response.data.status && response.data.orders) {
          const formattedOrders = response.data.orders.map((order) => {
            ...
            return {
              ...
              ), // ✅ Hide button if order is delivered
            };
          });          
    ...
    // Function to handle tracking click
    const handleTrackOrder = (orderId) => {
      // Remove existing track_order_id and set the new one
                ...
                {order.status === "cancelled" ? (
                  ...
                  <>
                    {order.showTrackButton && (
                      <Link href={`/track-orders/${order.id}`} onClick={() => handleTrackOrder(order.id)}>
