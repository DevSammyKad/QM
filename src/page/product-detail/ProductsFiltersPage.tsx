...
export default function ProductsFilterPage() {
  ...
  const addParamsHandler = (value: any, filterType: string) => {
    setSelectedFilters((prevFilters) => {
      ...
      fetchMedicines(searchQuery, sortBy, newFilters); // Fetch with updated filters
      return newFilters;
    });
  };
  ...
  const fetchMedicines = async (
    ...
    try {
      ...
      if (sortBy !== null) queryParams.append("sortBy", sortBy.toString()); // Sorting order (1 for ASC, other for DESC)
      ...
      const res = await fetch(`${Api.Medicine}?${queryParams.toString()}`, {
        headers: header,
      });
  ...
  // Handle sorting change (ASC or DESC)
  const handleSortChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const sortOption = parseInt(e.target.value, 10);
    setSortBy(sortOption);
    fetchMedicines(searchQuery, sortOption, selectedFilters); // Fetch medicines based on new sort
  };
